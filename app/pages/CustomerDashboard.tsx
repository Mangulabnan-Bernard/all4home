...
import { getCategories, getProviders, getUsers } from '../../lib/actions';
import { ServiceCategory, ProviderProfile, User } from '../../lib/types';
...
const CustomerDashboard: React.FC<CustomerDashboardProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (pros.length > 0) {
        const pro = pros[0];
        const user = users.find((u: User) => u.id === pro.userId);
