...
import { toggleDarkMode } from '../lib/actions';
...
const Layout = ({ children, user, onLogout, onServiceSelect, onUpdateUser }: LayoutProps): React.ReactElement => {
  ...
  const getCurrentPage = (path: string) => {
    ...
    if (path === '/manage-jobs') return 'manage-jobs';
    if (path === '/profile') return 'profile';
    if (path === '/my-jobs') return 'my-jobs';
    if (path === '/service-results') return 'service-results';
    if (path.startsWith('/public-profile/')) return 'public-profile';
    return 'dashboard';
  };
  ...
  const navigateTo = (page: string) => {
    if (page === 'dashboard' || page === 'search') router.push('/');
  ...
  const mobileNavItems = [
    ...
    { id: 'my-jobs', label: 'Bookings', icon: Calendar, roles: [UserRole.CUSTOMER] },
    { id: 'manage-jobs', label: 'Jobs', icon: Briefcase, roles: [UserRole.PROVIDER] },
    { id: 'admin-panel', label: 'Admin', icon: ShieldCheck, roles: [UserRole.ADMIN] },
      ...
      `}>
        <div className="h-full flex flex-col">
          <div className="p-8 flex items-center justify-between">
            <div className="flex items-center space-x-4 cursor-pointer" onClick={() => navigateTo('dashboard')}>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
             <div className="relative hidden sm:block" ref={menuRef}>
                ...
                <ServiceMenu isOpen={isServiceMenuOpen} onSelect={(name, category) => { router.push(`/service-results?service=${encodeURIComponent(name)}&category=${encodeURIComponent(category)}`); setIsServiceMenuOpen(false); }} onClose={() => setIsServiceMenuOpen(false)} />
             </div>
             ...
             {user ? (
               <div className="flex items-center space-x-4 pl-4 border-l dark:border-gray-800 cursor-pointer" onClick={() => navigateTo('profile')}>
                ...
                <button onClick={() => navigateTo('login')} className="text-sm font-black text-blue-600 dark:text-blue-400 px-6 py-3 border-2 border-transparent hover:border-blue-600 rounded-2xl transition-all">Sign In</button>
             )}
          </div>
        </div>
      </header>
